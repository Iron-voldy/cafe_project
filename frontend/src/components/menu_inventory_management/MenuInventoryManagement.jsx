...
const MenuInventoryManagement = () => {
    ...
    const handleMenuSubmit = async (e) => {
        ...
        try {
            ...
            fd.append('isAvailable', menuForm.isAvailable);
                        ...
                        {activeTab === 'menu' && (
                            <div className="relative shrink-0">
                                ...
                                <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="pl-7 pr-3 py-2.5 bg-white border border-warm/50 rounded-lg text-xs focus:outline-none focus:border-secondary transition-colors appearance-none cursor-pointer">
                                    {categories.map(c => <option key={c} value={c}>{c === 'all' ? 'All Categories' : c.charAt(0).toUpperCase() + c.slice(1).replace('_', ' ')}</option>)}
                                </select>
                            </div>
                        )}
            ...
            {loading ? (
                ...
                filteredMenu.length === 0 ? (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center p-10 text-center bg-white rounded-2xl border-2 border-dashed border-amber-200 my-8">
                        ...
                        <h3 className="text-base font-bold text-amber-900 mb-1">No Menu Items Found</h3>
                        <p className="text-sm text-amber-900-light/50 mb-4">Start by adding your first menu item</p>
                        <button onClick={() => { setEditingItem(null); setMenuForm({ name: '', description: '', category: 'beverage', price: '', isAvailable: true }); setShowModal(true); }} className="bg-amber-600 hover:bg-amber-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-colors flex items-center justify-center gap-2 py-2.5 px-6 shadow-sm hover:scale-[1.02] transition-transform duration-200 mt-2"><FaPlus size={14} /> Add Menu Item</button>
                    </motion.div>
                ) : (
                ...
                filteredStock.length === 0 ? (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center p-10 text-center bg-white rounded-2xl border-2 border-dashed border-amber-200 my-8">
                        ...
                        <h3 className="text-base font-bold text-amber-900 mb-1">No Stock Items Found</h3>
                        <p className="text-sm text-amber-900-light/50 mb-4">Add ingredients and supplies to track</p>
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-xl shadow-2xl w-full max-w-md">
                            ...
                            <form onSubmit={handleMenuSubmit} className="p-5 space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div><label className="block text-xs font-medium text-amber-900 mb-1">Category</label><select value={menuForm.category} onChange={(e) => setMenuForm({ ...menuForm, category: e.target.value })} className="w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all"><option value="beverage">Beverage</option><option value="appetizer">Appetizer</option><option value="main_course">Main Course</option><option value="dessert">Dessert</option><option value="snack">Snack</option><option value="special">Special</option></select></div>
                                ...
                                <div><label className="block text-xs font-medium text-amber-900 mb-1">Image</label><input type="file" accept="image/*" onChange={(e) => setImageFile(e.target.files[0])} className="w-full text-sm file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 cursor-pointer" />{editingItem?.image && !imageFile && <p className="text-[10px] text-gray-400 mt-1">Current image will be kept if no new file selected</p>}</div>
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-xl shadow-2xl w-full max-w-md">
                            ...
                            <form onSubmit={handleStockSubmit} className="p-5 space-y-4">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    ...
                                    <div><label className="block text-xs font-medium text-amber-900 mb-1">Unit</label><select value={stockForm.unit} onChange={(e) => setStockForm({ ...stockForm, unit: e.target.value })} className="w-full px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all"><option value="kg">Kg</option><option value="g">Grams</option><option value="l">Liters</option><option value="ml">mL</option><option value="pieces">Pieces</option><option value="packets">Packets</option></select></div>
